...
class ResultsDashboardCard(QFrame):
    ...
    def _setup_ui(self) -> None:
        ...
        struct_title = QLabel("Variables y Pivotes")
        struct_title.setStyleSheet(
            f"font-weight: 600; font-size: 18px; color: {COLOR_TEXT_PRIMARY}; margin-top: 4px;"
        )
        layout.addWidget(struct_title)

        self.structure_card = QFrame()
        self.structure_card.setStyleSheet(f"""
            QFrame {{
                background-color: {COLOR_SURFACE_INNER};
                border-radius: 6px;
                border: 1px solid {COLOR_SURFACE_ELEVATED};
                border-left: 4px solid {COLOR_FEEDBACK_SUCCESS};
            }}
        """)
        self.structure_layout = QVBoxLayout(self.structure_card)
        self.structure_layout.setContentsMargins(12, 10, 12, 10)
        self.structure_layout.setSpacing(6)
        layout.addWidget(self.structure_card)
        self._init_structure_card()
    ...
    def _init_structure_card(self) -> None:
        self._clear_layout(self.structure_layout)
        placeholder = QLabel("Esperando resolución")
        placeholder.setStyleSheet(
            f"color: {COLOR_TEXT_MUTED}; font-style: italic; font-size: 15px;"
        )
        self.structure_layout.addWidget(placeholder)
    ...
    def display_results(
        ...
        if analysis.system_type == SystemType.CONSISTENT_DETERMINED:
            ...
            border_accent = COLOR_FEEDBACK_SUCCESS
            ...
            text_color = COLOR_BG_BASE
            border_accent = COLOR_ACCENT_WARNING
        else:
            badge_color = COLOR_FEEDBACK_ERROR
            badge_text = "Sistema inconsistente"
            text_color = COLOR_TEXT_PRIMARY
            border_accent = COLOR_FEEDBACK_ERROR

        self.status_badge.setText(badge_text)
        self.status_badge.setStyleSheet(self._badge_style(badge_color, text_color))
        self.summary_label.setText(
            to_html_subscripts(prettify_math_text(analysis.summary_message), sub_size_px=11)
        )

        # Actualizar tarjeta de Variables y Pivotes
        self.structure_card.setStyleSheet(f"""
            QFrame {{
                background-color: {COLOR_SURFACE_INNER};
                border-radius: 6px;
