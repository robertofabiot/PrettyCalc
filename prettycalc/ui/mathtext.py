...
def variable_html(index_0: int, sub_size_px: Optional[int] = None, italic_x: bool = True) -> str:
    ...
    Ejemplo: <i>x</i><sub style='font-size:11px;'>1</sub> o <i>x</i><sub style='font-size:65%;'>1</sub>
    """
    sub_style = f"font-size:{sub_size_px}px;" if sub_size_px else "font-size:65%;"
    x_part = "<i>x</i>" if italic_x else "x"
    return f"{x_part}<sub style='{sub_style}'>{index_0 + 1}</sub>"
...
def to_html_subscripts(text: str, sub_size_px: Optional[int] = None, italic_x: bool = True) -> str:
    ...
    def _repl(m: re.Match) -> str:
        ...
        return f"{x_part}<sub style='{sub_style}'>{digits}</sub>"
